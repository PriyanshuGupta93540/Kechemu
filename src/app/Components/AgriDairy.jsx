    ...
    const BusinessCard = ({ business, color, onPrev, onNext, currentIndex, totalItems, type }) => {
        ...
        return (
            <div className="relative bg-white rounded-3xl shadow-2xl overflow-hidden ">
                ...
                {/* Header Section */}
                <div className={`relative ${isGreen ? 'bg-gradient-to-r from-green-50 to-green-100' : 'bg-gradient-to-r from-yellow-50 to-yellow-100'} p-4 sm:p-6 border-b border-gray-100`}>
                    <div className="flex flex-wrap items-center justify-between gap-3">
                        <div className="flex items-center space-x-3 sm:space-x-4 min-w-0">
                            <div className={`${isGreen ? 'bg-green-500' : 'bg-yellow-500'} p-2 sm:p-3 rounded-2xl shadow-lg flex-shrink-0`}>
                                {type === 'agritech' ? (
                                    <Sprout className="w-6 h-6 sm:w-8 sm:h-8 text-white" />
                                ) : (
                                    <Milk className="w-6 h-6 sm:w-8 sm:h-8 text-white" />
                                )}
                            </div>
                            <div className="min-w-0">
                                <h2 className="text-lg sm:text-2xl font-bold text-gray-800 break-words">{business.title}</h2>
